...
def visualize_one_batch(data_loaders, max_n: int = 5):
    ...
    # images = torch.abs(torch.imag(images))
    
    M = 256
    
    images_real = torch.real(images)/M
    images_real = torch.tanh(images_real)
    images_real = torch.permute(images_real, (0, 2, 3, 1))
    
    images_imag = torch.imag(images)/M
    images_imag = torch.tanh(images_imag)
    images_imag = torch.permute(images_imag, (0, 2, 3, 1))
    
    images_abs = torch.abs(images)/M
    images_abs = torch.tanh(images_abs)
    images_abs = torch.permute(images_abs, (0, 2, 3, 1))
    
    print(torch.max(images_real))
    print(torch.max(images_imag))
    print(torch.max(images_abs))
    

    # plot the images in the batch, along with the corresponding labels
    fig1 = plt.figure(figsize=(25, 4))
    for idx in range(max_n):
        ax = fig1.add_subplot(1, max_n, idx + 1, xticks=[], yticks=[])
        ax.imshow(images_real[idx])
    ...
    for idx in range(max_n):
        ax2 = fig2.add_subplot(1, max_n, idx + 1, xticks=[], yticks=[])
        ax2.imshow(images_imag[idx])
        # print out the correct label for each image
        # .item() gets the value contained in a Tensor
        ax2.set_title(class_names[labels[idx].item()])
        
    # plot the images in the batch, along with the corresponding labels
    fig3 = plt.figure(figsize=(25, 4))
    for idx in range(max_n):
        ax3 = fig3.add_subplot(1, max_n, idx + 1, xticks=[], yticks=[])
        ax3.imshow(images_abs[idx])
        # print out the correct label for each image
        # .item() gets the value contained in a Tensor
        ax3.set_title(class_names[labels[idx].item()])
...
def test_data_loaders_output_type(data_loaders):
    ...
    assert images[0].shape[-1] == 256, "The tensors returned by your dataloaders should be 256x256. Did you " \
